...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/12 21:06:53 by djustino          #+#    #+#             */
/*   Updated: 2024/05/12 21:50:21 by djustino         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_mutex_lastmeal(t_dinnertable *tabdinner)
{
	...
	{
		if (pthread_mutex_init(&tabdinner->philo[i].last_meal_flag, NULL))
			break ;
		i++;
	}
	...
	{
		while (i >= 0)
			pthread_mutex_destroy(&tabdinner->philo[i--].last_meal_flag);
		return (TRUE);
	}
	return (FALSE);
}
